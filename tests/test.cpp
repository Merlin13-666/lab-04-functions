...
std::vector<float> v1 = {-1, -2,- 3, -4, -5, 0};
auto pair1 = minMax(v1);
EXPECT_NEAR(pair1.first, -5, std::numeric_limits<float>::epsilon());
EXPECT_NEAR(pair1.second, 0, std::numeric_limits<float>::epsilon());
// массив из одинаковых элементов
std::vector<float> v2 ={1, 1, 1, 1, 1, 1};
auto pair2 = minMax(v2);
EXPECT_NEAR(pair2.first, 1, std::numeric_limits<float>::epsilon());
EXPECT_NEAR(pair2.second, 1, std::numeric_limits<float>::epsilon());

// массив из одного элемента
std::vector<float> v3 ={15};
auto pair3 = minMax(v3);
EXPECT_NEAR(pair3.first, 15, std::numeric_limits<float>::epsilon());
EXPECT_NEAR(pair3.second, 15, std::numeric_limits<float>::epsilon());

// пустой массив
std::vector<float> v4 ={};
auto pair4 = minMax(v4);
EXPECT_NEAR(pair4.first, std::numeric_limits<float>::min(), std::numeric_limits<float>::epsilon());
EXPECT_NEAR(pair4.second, std::numeric_limits<float>::max(), std::numeric_limits<float>::epsilon());
}
...
// отрицательные коэфиициенты
std::vector<float> v2 = {-11, -8, -36, -47, -2, -53, -56};
index_max = argmax(v2);
EXPECT_EQ(index_max, 4);
// положительные и отрицательные коэфиициенты
std::vector<float> v3 = {5, -3, -5, 47, -40, 3, 0};
index_max = argmax(v3);
